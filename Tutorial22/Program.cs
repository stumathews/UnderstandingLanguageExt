...
namespace Tutorial22
{
    ...
    // Note the function wen called on a list of monads is BindT or BiMapT
    class Program
    {
        static void Main(string[] args)
        {
            ...
            IEnumerable<Either<char, string>> result = listOfEithers.BiMapT(rightString => $"TurnAllRightStringToThis ({rightString})", leftInteger => 'A' /*Turn all left values in the eithers to 'A'*/);

            // ok so now we've transformed the eithers in diffirent ways depending on their type, lets look at them
            ...
            // You can also transform the eithers in the list using a bind(not autoamtic lift)
